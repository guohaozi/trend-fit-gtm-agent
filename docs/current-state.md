...
- Verification status: `npm test` currently runs 117 passing Node tests (including route/page smoke tests in `tests/route-smoke.test.ts`, the SerpApi near-zero-demand guard, and the Google Trends trend-only query test in `tests/seo-keyword-provider.test.ts`); `npm run build` completes a successful Next.js production build. `.github/workflows/ci.yml` now runs `npm ci`, `npm test`, and `npm run build` on pushes to `main` and pull requests.
...
- Latest round (Claude) ran the real SerpApi Google Trends path for the first time (live key) and fixed a near-zero-demand bug it surfaced: a low/no-demand query makes SerpApi return an `error`, and the relative interest index produced a bogus `-100% trend_declining / verified` finding that pushed Timing down. The provider now emits no findings (plus a `notes` entry) when SerpApi reports no results; field-path assumptions and the fixture shape are confirmed correct against live data ("stanley cup" still yields 9 findings). Query construction is now fixed too: `collect` queries the trend term only (market → `geo`), not product+market+trend — verified live, "dubai chocolate" (geo=DE) returns 12 findings where the old long query returned 0. Remaining follow-up: Google Trends related_queries can include spam/noise that a relevance filter would clean up. `scripts/verify-serpapi.ts` is the reusable live-check harness (key stays in the caller's env).
- Previous round (Claude) added the first route + page smoke tests (`tests/route-smoke.test.ts`): the report download API plus all six pages are now exercised across cases/profiles and fallback inputs, closing the UI-regression gap where `/workspace` and every page had no test. 114 tests pass; product code unchanged (test-only global React shim for the classic JSX transform).

...
- Verification status: `npm test` currently runs 114 passing Node tests (including the first route- and page-level smoke tests in `tests/route-smoke.test.ts`, covering the report download API and every page route); `npm run build` completes a successful Next.js production build. `.github/workflows/ci.yml` now runs `npm ci`, `npm test`, and `npm run build` on pushes to `main` and pull requests.
...
- Latest round (Claude) added the first route + page smoke tests (`tests/route-smoke.test.ts`): the report download API plus all six pages are now exercised across cases/profiles and fallback inputs, closing the UI-regression gap where `/workspace` and every page had no test. 114 tests pass; product code unchanged (test-only global React shim for the classic JSX transform).

...
- Verification status: `npm test` currently runs 116 passing Node tests (including route/page smoke tests in `tests/route-smoke.test.ts` and the SerpApi near-zero-demand guard in `tests/seo-keyword-provider.test.ts`); `npm run build` completes a successful Next.js production build. `.github/workflows/ci.yml` now runs `npm ci`, `npm test`, and `npm run build` on pushes to `main` and pull requests.
